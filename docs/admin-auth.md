...
The admin dashboard also reads the current scrape run version from `${BACKEND_URL}/admin/import/current-version` during server rendering. That request uses the same `X-Admin-Internal-Secret` header and displays only the numeric `current_version` value, plus the next accepted incremental version.
