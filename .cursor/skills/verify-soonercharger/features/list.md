...
- **HTTP open.** Run `.cursor/skills/verify-soonercharger/helpers/verify curl /list`. Status is `200`. The body contains `All upcoming Superchargers`, `All Statuses`, `All Regions`, `Riverside Plaza`, `Harbor Market`, `Oak Ridge`, and `Mesa Verde`.
...
- **Region filter.** Choose `United States` in the region select. The grid shows the two US sites, `Riverside Plaza` and `Mesa Verde`.
